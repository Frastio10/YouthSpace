...
(async()=>{
	fetch('article.json').then(async(result, key)=>{
		...
		if (url == "all") {
			...
			$('.catefory-list').removeClass('active');
			$('.edu').addClass('active');
			$('#category-name').text('Education');
		} else if (url == "nature") {
			$('.catefory-list').removeClass('active');
			$('.nature').addClass('active');
			$('#category-name').text('Nature');

		} else if (url == "technology") {
			$('.catefory-list').removeClass('active');
			$('.tech').addClass('active');
			$('#category-name').text('Technology');

		} else if (url == "lifestyle") {
			$('.catefory-list').removeClass('active');
			$('.lifestyle').addClass('active');
			$('#category-name').text('Lifestyle');
			...
			data.forEach(async(item,key)=>{
				if ((key <= 4 && item.title.search(regexp) != -1) || (item.desc.search(regexp) != -1) || (item.author.search(regexp) != -1) || (item.category.search(regexp) != -1)) {
					...
					temp = temp + template
						...
						.replace('{link}', `<a href="view-news.html?news_id=${item.id}&category=${item.category}" class="btn btn-blue">Read more</a>`)
		...
		if(url == "all" && search_query == null){
			...
			randomValue.forEach(async(item,key)=>{
				...
				if(key <= 4){
					...
					.replace('{category}',category)
					.replace('{link}', `<a href="view-news.html?news_id=${randomValue[key].id}&category=${randomValue[key].category}" class="btn btn-blue">Read more</a>`)
				}
